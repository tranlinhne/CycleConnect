...
// Lấy danh sách tin nhắn có phân trang, ưu tiên chưa đọc lên đầu
...
<style>
    ...
    /* Hàng chưa đọc được tô sáng nhẹ */
    .unread-row {
...
<div class="container-fluid px-4">
    <div class="page-header">
        <h2><i class="fas fa-envelope-open-text me-2" style="color: #F57C00;"></i> Liên hệ khách hàng</h2>
        <a href="<?= BASE_URL ?>contacts/export.php" class="btn btn-orange">
    ...
    <div class="table-responsive">
        <table class="contact-table">
            <thead>
                <tr>
                    <th>ID</th><th>Họ tên</th><th>Email</th><th>Chủ đề</th><th>Trạng thái</th><th>Ngày gửi</th><th>Thao tác</th>
                </tr>
            </thead>
            <tbody>
            <?php foreach ($msgs as $m): ?>
                <tr class="<?= !$m['is_read'] ? 'unread-row' : '' ?>">
                    ...
                    <td data-label="Họ tên"><?= htmlspecialchars($m['fullname']) ?></td>
                    <td data-label="Email"><?= htmlspecialchars($m['email']) ?></td>
                    <td data-label="Chủ đề"><?= htmlspecialchars($m['subject']) ?></td>
                    <td data-label="Trạng thái">
                        <span class="read-badge <?= $m['is_read'] ? 'read-yes' : 'read-no' ?>">
                            <?= $m['is_read'] ? 'Đã đọc' : 'Chưa đọc' ?>
                        </span>
                    </td>
                    <td data-label="Ngày gửi"><?= date('d/m/Y H:i', strtotime($m['created_at'])) ?></td>
                    <td data-label="Thao tác">
                        <a href="view.php?id=<?= $m['id'] ?>" class="btn btn-outline-orange btn-sm">
                            <i class="fas fa-reply-all"></i> Xem / Trả lời
                        </a>
                    </td>
                </tr>
            <?php endforeach; ?>
